...
router.post("/create/marks", async (req, res) => {
    try {
        const { rollNumber, subjectName, marks, maxMarks } = req.body;

        console.log('Received Request - Roll Number:', rollNumber);
        console.log('Received Request - Subject Name:', subjectName);

        // Fetch student details from the student collection using roll number
        const student = await Student.findOne({ rollNumber: rollNumber.toString() });

        // Log the found student for debugging
        console.log('Found Student:', student);

        // Fetch subject details using the subject name
        const subject = await Subject.findOne({ name: subjectName });

        // Log the found subject for debugging
